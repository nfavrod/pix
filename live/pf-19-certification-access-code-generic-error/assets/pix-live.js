  ...
  exports.default = Ember.Component.extend({
    ...
    actions: {
      submit: function submit() {
        ...
        var inputCode = this.get('accessCode');

        if (inputCode.length > 0) {
          this.set('_loadingCertification', true);
          return this.get('store').createRecord('course', { accessCode: inputCode }).save().then(function (certificationCourse) {
            _this.get('onSubmit')(certificationCourse);
          }).catch(function (error) {
            if (error.errors[0].status === '404') {
              _this.set('displayInvalidAccessCodeError', true);
            } else {
